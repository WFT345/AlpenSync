...
// path core/crypto/src/main/kotlin/io/pcontacts/core/crypto/srp/SrpClient.kt

package app.alpensync.core.auth.srp

import app.alpensync.core.auth.util.expandHash
import app.alpensync.core.auth.util.fromLittleEndianBigInteger
...
/**
 * Client side of Proton's custom SRP-6a variant. Live-verified by
 * pcontacts against `ProtonMail/go-srp` and the production API. This is
 * NOT standard RFC 5054 — Proton uses:
 *
...
class SrpClient(
    ...
    private val privateExponentBits: Int = PRIVATE_EXPONENT_BITS,
) {
     ...
     *   signature-verified (see ProtonModulusVerifier).
    ...
    fun login(
        ...
        // M1 = expandHash(A_LE || B_LE || S_LE) — no identity/salt/N/g terms
        val M1 = expandHash(aLE + bLE + sLE)

        // M2 = expandHash(A_LE || M1 || S_LE) — raw S, not K
    ...
    /** Constant-time comparison of the server proof against the expected M2. */
    ...
    companion object {
        ...
        private const val PRIVATE_EXPONENT_BITS = 256
    }
}
